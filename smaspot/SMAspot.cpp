...
unsigned char undoc_code1;
unsigned char undoc_code2;
unsigned long undoc_code3;
unsigned long undoc_first;
unsigned long undoc_last;
//End Undocumented
...
int main(int argc, char **argv)
{
	...
	{
		getSunriseSunset(&cfg);

		if ((cfg.forceInq == 0) && (cfg.isLight == 0))
		{
			if (VERBOSE_NORMAL) puts("Nothing to do... it's dark. Use -finq to force inquiry.");
			return 0;
		}
	}
	...
	//Undocumented - For testing purposes only
	if ((undoc_code1 != 0) && (undoc_code2 != 0))
	{
		if ((rc = getInverterData(&invData, Undoc_Spot)) != 0)
			printf("getUndocumentedFeatures returned an error: %d\n", rc);
		else
		{
			if (VERBOSE_NORMAL) printf("getUndocumentedFeatures done.\n");
		}
	...
	// only if enabled in config and difference is more than 1 minute
	if (cfg.synchTime == 1) SynchInverterTime();

	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SoftwareVersion)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, TypeLabel)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, DeviceStatus)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		...
		{
			if ((rc = getInverterData(&invData, MaxACPower)) != 0)
				printf("getMaxACPower returned an error: %d\n", rc);
			else
			{
				if ((invData.Pmax1 == 0) && (rc = getInverterData(&invData, MaxACPower2)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, EnergyProduction)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, OperationTime)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotDCPower)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotDCVoltage)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotACPower)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotACVoltage)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotACTotalPower)) != 0)
	...
	for (int retries = 0; retries < 3; retries++)
	{
		if ((rc = getInverterData(&invData, SpotGridFrequency)) != 0)
...
//					printf("%s : %.3fkWh - %3.3fkWh\n", strftime_t(cfg.DateTimeFormat, mData[idx].datetime), mData[idx].totalkWh, mData[idx].daykWh);
			}
		puts("======");
		}
	...
	/*
	 * When upload is enabled and there is light, send data.
	 * Note: Using -finq to force inverter inquiry, does not force upload
	 */
	if ((cfg.upload == 1) && (cfg.isLight == 1))
	{
		if (cfg.PVoutput == 1) PVOutputExport(&cfg, &invData);
		/*
		TODO: Add other online systems here
		*/
	}
...
int parseCmdline(int argc, char **argv, Config *cfg)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		///////////////////////////////////////////////////////
		// Undocumented Features - For testing purposes only //
		///////////////////////////////////////////////////////
		// Undocumented - For testing purposes only
		...
		else if (strnicmp(argv[i], "-code1:", 7) == 0)
		{
			lValue = strtoul(argv[i]+7, &pEnd, 16);
			if ((lValue > 255) || (*pEnd != 0))
			{
				InvalidArg(argv[i]);
				return -1;
			}
			else
				undoc_code1 = (unsigned char)lValue;
		}

		// Undocumented - For testing purposes only
		else if (strnicmp(argv[i], "-code2:", 7) == 0)
		{
			lValue = strtoul(argv[i]+7, &pEnd, 16);
			if ((lValue > 255) || (*pEnd != 0))
			{
				...
				undoc_code2 = (unsigned char)lValue;
		}

		// Undocumented - For testing purposes only
		else if (strnicmp(argv[i], "-code3:", 7) == 0)
		{
			lValue = strtoul(argv[i]+7, &pEnd, 16);
			if (*pEnd != 0)
			{
				...
				undoc_code3 = (unsigned long)lValue;
		}

		// Undocumented - For testing purposes only
		else if (strnicmp(argv[i], "-first:", 7) == 0)
		{
			lValue = strtoul(argv[i]+7, &pEnd, 16);
			if (*pEnd != 0)
			{
				...
				undoc_first = (unsigned long)lValue;
		}

		// Undocumented - For testing purposes only
		else if (strnicmp(argv[i], "-last:", 6) == 0)
		{
			lValue = strtoul(argv[i]+6, &pEnd, 16);
			if (*pEnd != 0)
			{
				InvalidArg(argv[i]);
				return -1;
				...
				undoc_last = (unsigned long)lValue;
		}
		//////////////////////////////////
...
/*
int getDeviceSerial(InverterData *invData)
{
    if (DEBUG_NORMAL) puts("getDeviceSerial()");
	...
	do
	{
		...
		{
			pcktID++;
			writePacketHeader(pcktBuf, 0x01, 0x00, smaBTInverterAddressArray);
			writePacket(pcktBuf, 0x09, 0xE0, pcktID, 0, 0, 0);
			writeByte(pcktBuf, 0x80);
			writeLong(pcktBuf, 0x00000200);
			writeLong(pcktBuf, 0x00000000);
			writeLong(pcktBuf, 0x00000000);
			writePacketTrailer(pcktBuf);
			writePacketLength(pcktBuf);
		} while (!isCrcValid(pcktBuf[packetposition-3], pcktBuf[packetposition-2]));
		...
		else
		{
			...
			{
				validPcktID = 1;
				//TODO: this is the so-called SUSyID
				//invData->modelID = (unsigned short)get_short(pcktBuf + 55);
				invData->Serial = (unsigned long)get_long(pcktBuf + 57);
			}
			else
			{
				if (DEBUG_HIGHEST) printf("Packet ID mismatch. Expected %d, received %d\n", pcktID, pcktBuf[27]);
...
void getSunriseSunset(Config *cfg)
{
	time_t currenttime;
	time(&currenttime);
	cfg->sunrise = sunrise(cfg->latitude, cfg->longitude, currenttime);
	cfg->sunset = sunset(cfg->latitude, cfg->longitude, currenttime);
	//Fixes Issue 20: Version 2.0.1 has lost a grip of time
	if (cfg->sunrise > cfg->sunset) cfg->sunrise -= 86400;

	//Fixes Issue 25: Exporting/Collecting data cuts off a bit too early
	if ((currenttime > (cfg->sunrise - cfg->SunRSOffset)) && (currenttime < (cfg->sunset + cfg->SunRSOffset)))
		cfg->isLight = true;
	else
		cfg->isLight = false;
	...
	if (VERBOSE_NORMAL)
	{
		printf("sunrise: %s\n", strftime_t(cfg->DateTimeFormat, cfg->sunrise));
		printf("sunset : %s\n", strftime_t(cfg->DateTimeFormat, cfg->sunset));
	}
}
